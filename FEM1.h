  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      for(unsigned int q=0; q<quadRule; q++){
        ...
        //EDIT - Define Flocal.
        // Use if statement to distinguish the 2 f's
        if(prob == 1 || prob == 2)
          Flocal[A] += Area*h_e/2. * basis_function(A,quad_points[q]) * fbar*x * quad_weight[q];
        if(prob == 3 || prob == 4)
          Flocal[A] += Area*h_e/2. * basis_function(A,quad_points[q]) * fhat*x*(L-x) * quad_weight[q];
      }
    }
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int q=0; q<quadRule; q++){
      ...
      // First find exact solution u_exact for each problem
      switch(int(prob)){
        case 1:
          u_exact = -fbar/E/6. * x*x*x + ((g2-g1)/L+fbar*L*L/6./E) * x + g1;
          break;
        case 2:
          u_exact = -fbar/E/6. * x*x*x + (h/E/Area+fbar*L*L/2./E) * x + g1;
          break;
        case 3:
          u_exact = fhat/E/12. * x*x*x*x + -fhat*L/E/6. * x*x*x + ((g2-g1)/L-fhat*L*L*L/12./E) * x + g1;
          break;
        case 4:
          u_exact = fhat/E/12. * x*x*x*x + -fhat*L/E/6. * x*x*x + (h/E/Area+fhat*L*L*L/6./E) * x + g1;
          break;
      }
      l2norm += (u_exact-u_h)*(u_exact-u_h);
    }
  }
